...
@app.get("/")
async def root():
    return {"message": "Welcome to the Company Chatbot"}

# Assuming you already have this route since it's working
@app.get("/chat-widget")
async def chat_widget():
    # Your existing chat widget logic here
...
@app.get("/")
async def root():
    return {"status": "online", "message": "Chat service is running"}
...
async def proxy(path: str):
    ...
    target_url = f"https://deluxe-concha-eb7972.netlify.app/{path}"
    try:
